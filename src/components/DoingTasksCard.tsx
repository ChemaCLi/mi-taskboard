...
import { PlayCircle, Clock, Loader2, AlertTriangle } from 'lucide-react';
...
function TaskItem({ task, onTaskClick }: TaskItemProps) {
  ...
  return (
    <div
      ...
      {...attributes}
      {...listeners}
      className="p-3 bg-slate-700/50 rounded border border-slate-600 hover:border-blue-400/50 transition-colors cursor-grab active:cursor-grabbing"
    >
      <div className="space-y-2">
        <div className="flex items-start justify-between">
          <h4 
            className="text-white font-medium text-sm flex-1 cursor-pointer hover:text-blue-400 transition-colors"
...
export function DoingTasksCard() {
  ...
  // Get tasks and filter for DOING status
  ...
  if (missionData.tasks.isLoading) {
    return (
      <Card className="bg-slate-800/50 border-blue-400/30">
        ...
        <CardContent>
          <div className="space-y-2 max-h-48 overflow-y-auto min-h-[150px] p-2 rounded-lg bg-slate-800/20 border-2 border-dashed border-blue-400/30">
  ...
  return (
    <Card className="bg-slate-800/50 border-blue-400/30">
      ...
      <CardContent>
        <div
          ref={setNodeRef}
          className="space-y-2 max-h-48 overflow-y-auto min-h-[150px] p-2 rounded-lg bg-slate-800/20 border-2 border-dashed border-blue-400/30"
        >
          {doingTasks.map((task) => (
          ...
          {doingTasks.length === 0 && (
            <div className="text-center text-slate-500 py-6">
              <PlayCircle className="w-6 h-6 mx-auto mb-2 opacity-50" />
